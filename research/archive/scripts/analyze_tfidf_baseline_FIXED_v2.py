...
def main() -> None:
    ...
    print("-" * 78)
    cols = [
        "report_id", "final_sif_potential", "sif_probability",
        "prediction_at_0.5", "prediction_at_validation_threshold", "error_type",
    ]
    print(predictions[cols].sort_values("sif_probability", ascending=False).to_string(index=False))
    ...
    print("-" * 78)
    for _, row in predictions.sort_values("sif_probability", ascending=False).iterrows():
        print()
    ...
    print("-" * 78)
    print("This test set contains only 6 records, so these results are diagnostic, not a reliable generalization estimate.")
    print("The immediate purpose is to inspect false positives/negatives and establish a baseline before transformer experiments.")
