"""
RAKSHAK / SIF-Insight FastAPI backend v1.2
Persistent human-review storage.

Frozen analysis stack:
- Final RAKSHAK v1.2 model
- Exact 422-feature inference
- Evidence Engine v1.5
- Reviewer Policy v1.4

New:
- SQLite review store at data/reviews/rakshak_reviews.db
- POST /reviews
- GET  /reviews
- GET  /reviews/{report_id}
- GET  /review-stats

Run:
    uvicorn scripts.rakshak_api_v1_2:app --host 127.0.0.1 --port 8000
"""
...
app = FastAPI(
    ...
    version="1.2.0",
    description=(
        "SIF precursor triage and persistent HSSE reviewer workflow. "
        "AI output is reviewer support, not an autonomous safety decision."
    ),
)
...
class ReviewRequest(BaseModel):
    ...
    decision: str = Field(
        pattern="^(CONFIRMED|REJECTED|NEEDS_MORE_INFO)$"
    )
...
def init_db():
    with db_connect() as conn:
        ...
        conn.execute(
            """
            CREATE INDEX IF NOT EXISTS idx_reviews_report_id
            ON reviews(report_id)
            """
        )
        conn.execute(
            """
...
def predict(request: PredictRequest):
    try:
        return predict_records([
            BatchItem(report_id=request.report_id, description=request.description)
        ])[0]
    except Exception as exc:
    ...
    try:
        ...
        with db_connect() as conn:
            cur = conn.execute(
                """
                INSERT INTO reviews
                (report_id, decision, reviewer, note, created_at)
                VALUES (?, ?, ?, ?, ?)
                """,
                (
                    request.report_id,
    ...
    try:
        with db_connect() as conn:
            if report_id:
                rows = conn.execute(
                    """
                    SELECT id, report_id, decision, reviewer, note, created_at
                    FROM reviews
                    WHERE report_id = ?
                    ORDER BY id DESC
                    LIMIT ?
...
def review_stats():
    try:
        ...
        return {
            ...
            "by_decision": [dict(r) for r in rows],
            "by_reviewer": [dict(r) for r in reviewers],
            "store": str(DB_FILE),
        }
    except Exception as exc:
        raise HTTPException(status_code=500, detail=str(exc))
