...
MODEL_FILE=PROJECT_ROOT/"experiments"/"rakshak_final_v1.2"/"rakshak_final_model.joblib"
...
def main():
    ...
    labels["final_sif_potential"]=labels["final_sif_potential"].astype(str).str.upper().str.strip()
    ...
    df=test[["report_id"]].merge(
        ...
        on="report_id",how="left",validate="one_to_one"
    )
    ...
    if embeddings.shape!=(20,emb_dim):
        raise ValueError(f"Embedding matrix {embeddings.shape}; expected (20,{emb_dim})")
    ...
    status="PASS_EXACT" if max_diff<1e-5 else ("PASS_NUMERIC_TOLERANCE" if max_diff<1e-3 else "FAIL")

    result.to_csv(OUT_DIR/"final_refit_inference_consistency.csv",index=False,encoding="utf-8-sig")
    ...
    (OUT_DIR/"final_refit_summary.json").write_text(json.dumps(summary,indent=2),encoding="utf-8")
    ...
    print("\nPREDICTIONS")
    print(result.sort_values("absolute_difference",ascending=False).to_string(index=False))
    print("\nSaved:",OUT_DIR)
