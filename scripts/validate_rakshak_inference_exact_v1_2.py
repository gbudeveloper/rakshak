...
MODEL_PRED=PROJECT_ROOT/"experiments"/"rakshak_final_v1.2"/"locked_test_predictions_with_evidence.csv"
...
def main():
    ...
    data=(test[["report_id"]]
          ...
          .merge(saved[["report_id","sif_precursor_probability"]],on="report_id",how="left"))
    if len(data)!=20:
    ...
    for _,r in data.iterrows():
        ...
        emb=encoder.encode([text],convert_to_numpy=True,normalize_embeddings=False,
                           show_progress_bar=False,device=device)[0].astype(np.float32)
    ...
    status="PASS_EXACT" if max_diff<1e-5 else ("PASS_NUMERIC_TOLERANCE" if max_diff<1e-3 else "FAIL_SCHEMA_OR_RUNTIME_MISMATCH")
    summary={"status":status,"records":len(out),"max_absolute_difference":max_diff,
             "mean_absolute_difference":mean_diff,"embedding_dimension":emb_dim,
             "pathway_dimension":38,"total_dimension":422}
    od=PROJECT_ROOT/"experiments"/"rakshak_inference_validation_v1.2"
    od.mkdir(parents=True,exist_ok=True)
    out.sort_values("absolute_difference",ascending=False).to_csv(od/"locked_test_inference_consistency.csv",index=False)
    ...
    print("\nTOP DIFFERENCES")
    print(out.sort_values("absolute_difference",ascending=False).head(10).to_string(index=False))
    print("\nSaved:")
    print(od)
